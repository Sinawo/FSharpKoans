...
module ``14: List operations are so easy, you could make them yourself!`` =
    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int = __
           //let rec findlength n a =
           //      match // write a function to find the length of a list
   ...
   *)
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list = __
            //let rec filterodd nlist =
            //    match n% 2 <> 0 with 
            //    | false -> filterodd nlist
            //    | true -> filterodd nlist
    ...
    let ``16 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
              // write a function to do a fold.
                 let rec foldList xs n =
                   match xs with
                   | [] -> n
                   | a::b -> foldList b (f n a)
                 foldList xs initialState
